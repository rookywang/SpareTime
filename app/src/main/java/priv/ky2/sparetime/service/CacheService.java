package priv.ky2.sparetime.service;
...
public class CacheService extends Service {
    ...
    public void onCreate() {
        ...
        filter.addAction("pri.ky2.sparetime.LOCAL_BROADCAST");
        ...
        if (cursor.moveToFirst()) {
            do {
                if ((cursor.getInt(cursor.getColumnIndex("zhihu_id")) == id)
                    ...
                    StringRequest request = new StringRequest(Request.Method.GET, Urls.ZHIHU_NEWS + id, new Response.Listener<String>() {
                        @Override
        ...
        if (cursor.moveToFirst()) {
            do {
                if ((cursor.getInt(cursor.getColumnIndex("douban_id")) == id)
                    ...
                    StringRequest request = new StringRequest(Urls.DOUBAN_ARTICLE_DETAIL + id, new Response.Listener<String>() {
                        @Override
                        public void onResponse(String s) {
                            ContentValues values = new ContentValues();
        ...
        if (cursor.moveToFirst()) {
            do {
                if ((cursor.getInt(cursor.getColumnIndex("guokr_id")) == id)
                    ...
                    StringRequest request = new StringRequest(Urls.GUOKR_ARTICLE_LINK_V1 + id, new Response.Listener<String>() {
                        @Override
                        public void onResponse(String s) {
                            ContentValues values = new ContentValues();
