...
import static priv.ky2.sparetime.network.Urls.GUOKR_ARTICLE_LINK_V1;
...
public class DetailsPresenter implements DetailsContract.Presenter {
    ...
    public DetailsPresenter(@NonNull Context context, @NonNull DetailsContract.View view) {
        ...
        try {
            ...
            switch (type) {
                case TYPE_ZHIHU:
                    ...
                    break;
                case TYPE_GUOKE:
                    intent.setData(Uri.parse(GUOKR_ARTICLE_LINK_V1 + id));
                    break;
                case TYPE_DOUBAN:
        ...
        try {
            ...
            switch (type) {
                case TYPE_ZHIHU:
                    ...
                    break;
                case TYPE_GUOKE:
                    shareText += Urls.GUOKR_ARTICLE_LINK_V1 + id;
                    break;
                case TYPE_DOUBAN:
    ...
    public void openUrl(WebView webView, String url) {
        if (sp.getBoolean("in_app_browser",true)) {
            CustomTabsIntent.Builder customTabsIntent = new CustomTabsIntent.Builder()
                    ...
                    .setShowTitle(true);
            ChromeCustomTabActivityHelper.openCustomTab(
                    (Activity) context,
    ...
    public void copyLink() {
        ...
        switch (type) {
            case TYPE_ZHIHU:
                ...
                break;
            case TYPE_GUOKE:
                clipData = ClipData.newPlainText("text", Html.fromHtml(Urls.GUOKR_ARTICLE_LINK_V1 + id).toString());
                break;
            case TYPE_DOUBAN:
        ...
        switch (type) {
            case TYPE_ZHIHU:
                if (NetworkState.networkConnected(context)) {
                    model.load(Urls.ZHIHU_NEWS + id, new OnStringListener() {
                        @Override
            ...
            case TYPE_GUOKE:
                if (NetworkState.networkConnected(context)) {
                    model.load(GUOKR_ARTICLE_LINK_V1 + id, new OnStringListener() {
                        @Override
                        public void onSuccess(String result) {
            ...
            case TYPE_DOUBAN:
                if (NetworkState.networkConnected(context)) {
                    model.load(Urls.DOUBAN_ARTICLE_DETAIL + id, new OnStringListener() {
                        @Override
                        public void onSuccess(String result) {
                            try {
    ...
    private boolean checkNull() {
        return (type == BeanType.TYPE_ZHIHU && zhihuDailyStory == null)
                || (type == BeanType.TYPE_GUOKE && guokrStory == null)
